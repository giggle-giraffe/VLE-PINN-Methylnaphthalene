...
class MoleFractionLoss(nn.Module):
    ...
    - All outputs are softmax normalized, using log-MSE loss
    """
    def __init__(self, n_y_components, n_x_components=None, has_vf=False, epsilon=1e-8, 
        ...
        Args:
            ...
            has_vf: Whether VF is included as first output (False for reg_y)
            epsilon: Small constant for log stability
            vf_weight: Weight for VF loss term (unused for reg_y)
        ...
        # Calculate indices (for reg_y: no VF, only y*)
        if has_vf:
            self.vf_idx = 0
            self.y_start = 1
    ...
    def forward(self, pred, target):
        ...
        - VF: standard MSE (if present)
        - Mole fractions: MSE in log space
        """
        total_loss = 0.0
        
        # VF loss (standard MSE) - not applicable for reg_y
        ...
        # Liquid mole fractions (x*) - log-space MSE (if present, not for reg_y)
...
def train_model(config, model_save_path):
    ...
    use_pretrained_scalers = os.path.exists(os.path.join(scaler_dir, 'reg_y_feature_scaler.pkl'))
    ...
    if data_loading_mode == 'classic':
        ...
        if use_pretrained_scalers and not skip_target_scaling:
            ...
            feature_scaler, target_scaler = load_pretrained_scalers(scaler_dir, 'reg_y')
            
            full_dataset = InMemoryThermoRegressionDataset(
                hdf5_file=data_file_path,
                target_cols=config['data']['target_cols'],
                phase_filter='single_phase',  # For vapor phase (y* variables only)
            ...
            feature_scaler_path = os.path.join(scaler_dir, 'reg_y_feature_scaler.pkl')
    ...
    if grad_summary:
        ...
        plot_individual_gradient_flow(
            model_name=f"{config['model']['name']}_reg_y",
...
def main():
    parser = argparse.ArgumentParser(description='Train Regression Neural Network (Y - Vapor Phase)')
    parser.add_argument('--config', type=str, default='phase/config/training_reg_y_mn.yaml',
    ...
    setproctitle.setproctitle(f"phase_{args.system}_train_reg_y")
    ...
    model_save_path = os.path.join(
        args.output_dir,
        f"{config['model']['name']}_{model_version}_reg_y"
    )
